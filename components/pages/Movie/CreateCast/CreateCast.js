...
const CreateCast = (props) => {
    ...
    return <form method="POST" onSubmit={handleCreate}>
        <Layout>
            <Stack spacing={4}>
                <Stack direction='row' alignItems="center" spacing={4}>
                    <Text size="medium" color="primary">Cadastro de Elenco</Text>
                </Stack>
                <Stack spacing={2}>
                    <Stack direction="row" spacing={4}>
                        <FormControl sx={{ minWidth: "200px"}}>
                            <InputLabel id="select-cast" size="small">Elenco</InputLabel>
                            <Select
                                labelId="select-cast"
                                id="select-cast-id"
                                label="Elenco"
                            ...
                            >
                                 {listTypeCasts.map((cast, idx) => (
                                    <MenuItem key={idx} value={cast.id}>{cast.description}</MenuItem>
                                ))}
                            </Select>
                        </FormControl>
                        <TextField type="input" label='Nome'  sx={{ minWidth: "300px"}} size="small" name="name" />
                        <TextField type="input" label='Nome do Personagem' sx={{ minWidth: "300px"}} size="small" name="characterName"/>
                    </Stack>
                    <Stack direction='row' spacing={2}>
                        <TextField type="input" label='Url da Foto' size="small" sx={{ minWidth: "700px"}} name="photo" />
                            ...
                            {castInMovie.map((row) => (
                                ...
                                >
                                    ...
                                    <TableCell align="left">
                                        { row.photo ? 
                                            <span style={{ cursor: "pointer" }} onClick={() => window.open(row.photo)}>
                                                <Image src={cameraImage.src} width={16} height={16} alt="Ver foto" />
                                            </span> : 
                                            '-'
                                        }
                                    </TableCell>
                                    <TableCell align="left">
                                        <span style={{ cursor: "pointer" }} onClick={() => handleDelete(row.id)}>
                                            <Image src={deleteImage.src} width={16} height={16} alt="Excluir Gênero" />
                                        </span>
                                    </TableCell>
                                </TableRow>
                            ))}
