...
const DynamicCreateCast = dynamic(
  () => import("../../../components/pages/Movie/CreateCast"),
  {
    ssr: false,
  }
);
...
export async function getServerSideProps(ctx) {
  ...
  let castInMovie = [];
  const responseGenresInMovie = await fetch(
    `${movieApiBaseUrl}/v1/movie/cast/${idMovie}`,
    {
      method: "GET",
      headers: {
  ...
  let listTypeCasts = [];
  const responseListGenres = await fetch(
    `${movieApiBaseUrl}/v1/cast-profile/list`,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
